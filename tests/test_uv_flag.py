...
def test_upgrade_with_uv_flag(runner):
...
def test_verify_with_uv_flag(runner, monkeypatch):
...
def test_lock_without_uv_flag(runner):
...
def test_upgrade_without_uv_flag(runner):
...
def test_verify_without_uv_flag(runner, monkeypatch):
...
def test_uv_not_available(runner, monkeypatch):
    ...
    monkeypatch.setattr(
        "pipcompilemulti.environment.Environment.check_uv_available", mock_check_uv
    )
    result = runner.invoke(cli, ["upgrade", "--uv"])
    assert result.exit_code != 0
...
def test_uv_available(runner, monkeypatch):
    """Test behavior when UV is available"""

    # Mock check_uv_available to return True
    def mock_check_uv():
        return True
    ...
    monkeypatch.setattr(
        "pipcompilemulti.cli_v2.run_configurations", mock_run_configurations
    )
    result = runner.invoke(cli, ["upgrade", "--uv"])
    assert result.exit_code == 0
    assert FEATURES.use_uv.get() is True
